...
**A) AI 生图** — `image_gen.py`。设置 `IMAGE_BACKEND` 和对应 `*_API_KEY`（`OPENAI_API_KEY`、`GEMINI_API_KEY`、`AGNES_API_KEY` 等），流程会自动调用。`python3 skills/ppt-master/scripts/image_gen.py --list-backends` 查看完整后端清单。Agnes AI 现在可以直接通过 `IMAGE_BACKEND=agnes` 作为正式后端使用；OpenAI 路径下 `gpt-image-2` 仍然是当前综合质量最佳的默认选项。

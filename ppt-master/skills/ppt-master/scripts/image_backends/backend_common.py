...
def download_image(url: str, path: str, headers: dict = None, timeout: int = 180) -> str:
    """Download an image URL and save it to disk, with retries for transient transport errors."""
    last_error = None
    for attempt in range(MAX_RETRIES + 1):
        try:
            ...
            print(f"  [WARN] Download error: {exc}. Retrying in {delay}s...")
            time.sleep(delay)
    raise RuntimeError(f"Failed to download generated image after {MAX_RETRIES + 1} attempts: {last_error}")

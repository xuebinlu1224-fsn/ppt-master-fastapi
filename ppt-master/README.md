...
**A) AI generation** — `image_gen.py`. Set `IMAGE_BACKEND` plus the provider's `*_API_KEY` (`OPENAI_API_KEY`, `GEMINI_API_KEY`, `AGNES_API_KEY`, etc.), and the pipeline calls it automatically. Run `python3 skills/ppt-master/scripts/image_gen.py --list-backends` for the full backend list. Agnes AI is now available as a dedicated backend via `IMAGE_BACKEND=agnes`; `gpt-image-2` remains the best default for the OpenAI path.
